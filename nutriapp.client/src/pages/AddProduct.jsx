    ...
    const getProducts = async () => {
      try {
        const response = await axios.get(
          `${apiUrl}/api/Product/userProducts?PageNumber=${pageNumber}&PageSize=3`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleAddProduct = async (productId, grams) => {
    try {
      const response = await axios.put(
        `${apiUrl}/api/Dish/${dishId}/addProduct?productId=${productId}&grams=${grams}`,
        null,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="wrapper">
      ...
      <div className="productCreate2">
          ...
          {userProducts.length !== 0 ? (
            userProducts.map((item) => (
              <div className="userProduct" key={item.id}>
                ...
                <li>
                  <i>Marka: {item.brand}</i>
                </li>
                <li>Kalorie: {item.calories}</li>
                <li>
                  <small>
                    Białka: {item.proteins}, Węglowodany: {item.carbohydrates},
                    Tłuszcze: {item.fats}
                  </small>
                </li>
                ...
                <div className="productButtons">
                  <button
                    onClick={() =>
                      handleAddProduct(item.id, item.gramsInPortion)
                    }
                  >
                    Dodaj
                  </button>
        ...
        <button onClick={handleNextPage} disabled={pageNumber === pages}>
          Następna strona
        </button>
        <p>
          {pageNumber}/{pages}
