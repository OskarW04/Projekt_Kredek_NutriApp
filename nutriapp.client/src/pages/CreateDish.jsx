...
export function CreateDish() {
  ...
  useEffect(() => {
    const getProducts = async () => {
      try {
        ...
        const allProducts = response.data.dishProducts.concat(
          response.data.dishApiProducts
        );
        setProducts(
          allProducts.map((dish) => ({
            name: dish.name,
            id: dish.id || dish.apiId,
            brand: dish.brand,
            calories: dish.calories,
            carbohydrates: dish.carbohydrates,
            fats: dish.fats,
            proteins: dish.proteins,
            gramsInPortion: dish.gramsInPortion,
          }))
        );
      } catch (error) {
        console.error(error);
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      await axios.delete(
        `${apiUrl}/api/Dish/${Id}/removeProduct?productId=${productId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        {Array.isArray(products) ? (
          products.map((product, index) => (
            <div className="dishProducts" key={index}>
              ...
              <li>
                <i>Marka: {product.brand !== null ? product.brand : "Brak"}</i>
              </li>
              <li>Kalorie: {product.calories}</li>
              <li>
                <small>
                  Białka: {product.proteins}, Węglowodany:{" "}
                  {product.carbohydrates}, Tłuszcze: {product.fats}
                </small>
              </li>
